...
open class HTTPFileProvider: NSObject, FileProviderBasicRemote, FileProviderOperations, FileProviderReadWrite, FileProviderReadWriteProgressive {
    ...
    private var backgroundSessionDelegate: SessionDelegate?
    
    // 后台任务的完成回调，需要存储因为回调可能在另一个进程中发生
    private var backgroundCompletionHandlers: [String: SimpleCompletionHandler] = [:]
    
    // 临时URL到目标URL的映射，用于后台完成时移动文件
    private var backgroundDestURLs: [URL: URL] = [:]
     ...
     */
    public init(baseURL: URL?, credential: URLCredential?, cache: URLCache?, backgroundIdentifier: String? = nil) {
        ...
        if let identifier = backgroundIdentifier {
            setupBackgroundSession(withIdentifier: identifier)
        }
    }
    ...
    open func copyItem(path: String, toLocalURL destURL: URL, completionHandler: SimpleCompletionHandler) -> Progress? {
        ...
        guard backgroundSession == nil else {
            // 使用后台会话下载
            return downloadWithBackgroundSession(path: path, toLocalURL: destURL, completionHandler: completionHandler)
        }
    ...
    */
    ...
    // 设置后台会话
    private func setupBackgroundSession(withIdentifier identifier: String) {
        let config = URLSessionConfiguration.background(withIdentifier: identifier)
        self.backgroundSessionDelegate = SessionDelegate(fileProvider: self)
        self._backgroundSession = URLSession(configuration: config, 
                                           delegate: backgroundSessionDelegate, 
                                           delegateQueue: self.operation_queue)
    }
    
    // 后台会话访问器
    private var backgroundSession: URLSession? {
        if _backgroundSession == nil, let identifier = backgroundIdentifier {
            setupBackgroundSession(withIdentifier: identifier)
        }
    ...
    public func handleEventsForBackgroundURLSession(identifier: String, completionHandler: @escaping () -> Void) {
        guard identifier == backgroundIdentifier else { return }
        
        // 存储系统提供的completionHandler，后续在会话完成时调用
        backgroundSessionCompletionHandler = completionHandler
    }
    
    // 这个属性由AppDelegate存储并调用
    private var backgroundSessionCompletionHandler: (() -> Void)?
    
    // 添加使用后台会话下载的方法
    private func downloadWithBackgroundSession(path: String, toLocalURL destURL: URL, completionHandler: SimpleCompletionHandler) -> Progress? {
        guard let backgroundSession = self.backgroundSession else {
            completionHandler?(NSError(domain: "FileProviderError", 
                                      code: -1, 
                                      userInfo: [NSLocalizedDescriptionKey: "无法创建后台会话"]))
            return nil
        }
        ...
        backgroundCompletionHandlers[taskId] = completionHandler
        ...
        // 设置取消处理
        progress.cancellationHandler = { [weak downloadTask] in
            downloadTask?.cancel()
        }
        
        // 启动任务
        downloadTask.taskDescription = (operation.json ?? <#default value#>) + "&taskId=\(taskId)"
        downloadTask.resume()
